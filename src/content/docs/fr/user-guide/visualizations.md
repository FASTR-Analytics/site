...
Pour les désagrégations par zone administrative et par établissement, vous pouvez inclure une ligne de total récapitulatif en cochant l'option correspondant à la désagrégation concernée. Le libellé affiché dépend du contexte : **Inclure les résultats nationaux** pour un total national par zone administrative, **Inclure les résultats de toutes les zones sélectionnées** lorsque la visualisation est limitée à une zone parente spécifique, **Inclure les résultats de tous les établissements** pour un total par établissement, ou **Inclure les résultats de tous les établissements sélectionnés** lorsque les établissements sont filtrés sur un sous-ensemble. Cette ligne de total apparaît au début ou à la fin du graphique ou du tableau selon le paramètre de position. L'option est disponible sur un niveau de zone administrative désagrégé (zone administrative 2, 3 ou 4) ou sur une colonne d'établissement, à condition que ce niveau ne soit pas affiché comme réplicant ou zone cartographique et ne soit pas filtré sur une seule valeur ; elle n'est pas non plus disponible sur les cartes. De plus, l'indicateur doit permettre l'agrégation - si ce n'est pas le cas, la case à cocher apparaît désactivée avec une explication. Une seule ligne de total récapitulatif peut être active à la fois.
