...
Préparez un fichier CSV au format large : une ligne par établissement, avec une colonne `facility_id` suivie d'une colonne par libellé de point temporel. Une cellule vide signifie que l'établissement n'est pas dans l'échantillon de ce tour et qu'aucune pondération ne sera enregistrée pour lui. Les en-têtes de colonnes de points temporels doivent correspondre exactement aux libellés déjà créés par une importation de données HFA.

Pour téléverser les pondérations, sélectionnez ou chargez votre CSV à l'aide du sélecteur de fichier, puis cliquez sur **Importer les pondérations**. FASTR valide chaque ligne. Les lignes comportant des identifiants d'établissement invalides sont rejetées avec un message d'erreur listant les premiers cas problématiques. Les cellules contenant des valeurs de pondération non positives sont également rejetées. Les cellules vides sont ignorées silencieusement - elles représentent des établissements absents de l'échantillon de ce tour. Une importation réussie indique combien de cellules de pondération ont été importées, combien de cellules vides ont été ignorées et quels points temporels ont été couverts. Un nouveau téléversement met à jour les pondérations existantes pour le même établissement et le même point temporel.

Le gestionnaire de pondérations affiche un résumé de couverture indiquant, pour chaque point temporel, combien d'établissements disposant de données ont une pondération correspondante. Un avertissement met en évidence les points temporels où certains établissements ont des données mais aucune pondération - ces établissements compteront avec une pondération de 1 lorsque l'analyse pondérée sera activée. Vous pouvez télécharger les pondérations actuelles au format CSV dans le même format large, les modifier et les réimporter.
